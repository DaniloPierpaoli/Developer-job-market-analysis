...
# Get top 20 countries by response count
...
selected_ages = st.sidebar.multiselect("Select Age Group(s)", age_groups, default=age_groups)
...
lang_df = pd.concat([worked, want, admired], axis=1).fillna(0).astype(int)
...
db_df = pd.DataFrame({
    "Worked With": db_worked,
    "Want To Work With": db_want,
    "Admired": db_admired
}).fillna(0).astype(int)
